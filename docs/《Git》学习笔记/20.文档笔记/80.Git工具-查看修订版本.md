...
```sh
$ git 41.log --abbrev-commit --pretty=oneline
...
```sh
$ git 41.log --pretty=format:'%h %s' --graph
...
```sh
$ git 41.log refA..refB
$ git 41.log ^refA refB
$ git 41.log refB --not refA
```

这个语法很好用，因为你可以在查询中指定超过两个的引用，这是双点语法无法实现的。 比如，你想查看所有被 `refA` 或 `refB` 包含的但是不被 `refC` 包含的提交，你可以使用以下任意一个命令：

```sh
$ git 41.log refA refB ^refC
$ git 41.log refA refB --not refC
```
...
```sh
$ git 41.log --left-right master...experiment
< F
< E
> D
> C
