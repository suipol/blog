...
```sh
$ git 41.01.日记 --abbrev-commit --pretty=oneline
...
```sh
$ git 41.01.日记 --pretty=format:'%h %s' --graph
...
```sh
$ git 41.01.日记 master..experiment
D
C
```
...
```sh
$ git 41.01.日记 experiment..master
F
E
```
...
```sh
$ git 41.01.日记 origin/master..HEAD
```
...
```sh
$ git 41.01.日记 refA..refB
$ git 41.01.日记 ^refA refB
$ git 41.01.日记 refB --not refA
```

这个语法很好用，因为你可以在查询中指定超过两个的引用，这是双点语法无法实现的。 比如，你想查看所有被 `refA` 或 `refB` 包含的但是不被 `refC` 包含的提交，你可以使用以下任意一个命令：

```sh
$ git 41.01.日记 refA refB ^refC
$ git 41.01.日记 refA refB --not refC
```
...
```sh
$ git 41.01.日记 --left-right master...experiment
< F
< E
> D
> C
